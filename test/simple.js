...
suite("API", () => {
  test("ToSpiderMonkey should exist", () => {
    assert.equal(typeof converters.toSpiderMonkey, "function");
  });

  test("ToSpiderMonkey should exist", () => {
    assert.equal(typeof converters.toLaserBat, "function");
  });
});
...
suite("General", function () {
  ...
  function roundTrip(type, source) {
    test(type, function () {
      ...
      stripRaw(smAst);
      var lbAst = converters.toLaserBat(smAst);
      var smAst2 = converters.toSpiderMonkey(lbAst);
